...
export default function DeleteAccount({ onLogout }) {
      ...
      <div className="login-card">
        ...
        {done ? (
          <div className="form" style={{ textAlign: "center" }}>
            ...
            <a className="btn" style={{ marginTop: 16 }} href="/login">
              로그인 페이지로 이동
            </a>
          </div>
        ) : (
          <form className="form" onSubmit={submit}>
            ...
            <input
              id="pwd" type="password" className="input" placeholder="비밀번호를 입력하세요"
              value={password} onChange={(e) => setPassword(e.target.value)} required
            />
            <div className={`hint ${password ? (pwdOk ? "success" : "error") : ""}`}>
              {password ? (checking ? "확인 중..." : pwdOk ? "일치합니다." : "일치하지 않습니다.") : "\u00A0"}
            </div>
